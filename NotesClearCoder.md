...
# Самообучение
* ката, итеративноя реализация простого програмного примера, оттачивая корректность каждого действия при реализации, и анализ оптимальности каждого действия
* Вадза - поочередная реализация тестов и кодов к ним, позволяет подглядеть интересные решения и обменятся опытом в решении кат, и задач в принцепе
* рандори - поочередная реализация кты, но роли после каждого теста меняются
* имеет смысл взять openSource проэкт на незнакомом языке, чтоб развиватся
* тренировки в личное время
...
# Тесты
Тесты можно разделить на такие группы:
* Иследовательские тесты - это форма тестирования, основанная на интуиции и опыте, которая помогает обнаружить неожиданные проблемы и дефекты в программном обеспечении.
* Системные тесты - это форма тестирования программного обеспечения, проводимая на уровне готовой системы в целом. Они направлены на проверку соответствия системы её требованиям и ожиданиям пользователей. В отличие от компонентных и интеграционных тестов, системные тесты оценивают функциональность системы в целом, включая её интерфейс, производительность, безопасность и другие аспекты. (ближе к конкретным техническим характеристекам чем приемные)
* Интеграционные тесты - это форма тестирования программного обеспечения, направленная на проверку взаимодействия и совместной работы различных компонентов или модулей системы. Целью интеграционных тестов является обнаружение ошибок, связанных с интерфейсами и взаимодействием между компонентами, а также проверка корректности данных, передаваемых между ними.
* Компонентные тесты - это форма тестирования программного обеспечения, в которой отдельные компоненты или модули программы тестируются независимо от других компонентов. Целью таких тестов является проверка корректности функционирования отдельных частей программы, обычно без их интеграции с другими компонентами.
* Модульные тесты - это тесты, которые проверяют отдельные компоненты (модули) программного кода на корректность их работы независимо от других частей программы. (самый низкий уровень)

Наша цель как разработчика тестов минимизировать нагрузку на тестеровщиков и из всего данного списка оставить только иследовательские тесты

## Модульные тесты
### TDD
#### Три закона TDD
* Новый рабочий код пишется только после того как будет написан тест который не проходит текущий код
* Реализуется ровно такой обем теста который необходим для того чтоб тест не проходил (если нет компиляции то тоже считается)
* Реализуется только необходимый для прохождения текущего теста код
весь процесс должен укладыватся в 30 секунд

#### Цикл «красный/зеленый/рефакторинг»
Является описанием к процессу TDD
* красный - реализуется минимальный тест который не проходит при текущей реализации функциональности
* зеленый - реализуется функциональность которая позволяет пройти тест
* рефакторинг - проводится рефакторинг теста и реализованой функциональности, при этом за счет того что она покрыта тестами, будет сложно налажать

#### преймущества:
* код задокументирован
* код является покрытым тестами, появляется увереность в корректности работы
* код является легко изменяемым, и толерантным к рефактроингу (если чтото работает не корректно, тесты подскажут о проблеме)
* Архитектура, необходимость реализации тестов мотевирует дизайнить архитектуру качественее (требует изолирования и независимости разных сущьностей)

## приемочные тесты


# Комуникация с заказщиком (разработка требований)
## Формулировка требований
* В требованиях не должно быть белых пятен (все спорные моменты или моменты которые могут троктоватся по разному должны быть обговорены, четко сформулированы и задокументированы) (приемочные тесты могут помоч)
* Приемочные тесты должны помоч сделать требования более прозрачными и уточнить тонкие моменты.(так как сделают их явными)

## Оценка времени выполнения задачи
* Факторы мешающие точности оценки
** При разработке требований всем не хватает точности, пользователю нужны сроки. Разработчику нужно четкое ТЗ (и одно влеяет на другое)
** При разработки требований. ВО время проработки требований заказщик получает больше информации, меняет свое восприятие что влеяет на требования, в результате чего они теряют актуальность. (При четкой формулировке требований, мы получим узкие рамки, и по этому данное требование быстро потеряет актуальность)/
как вывод получим проблему что по мере выполнения проэкта когда заказщик видет результат, требования меняются
* изза приведеных выше факторов нельзя давать точную оценку, необходима оценка с диапозоном неопределнности который позволит невелировать неточности